Describe "Get-HostsFileObject Tests" {
    ...
    Context "Tests - HostsFileObject" {
        ...
        It "Test - 'HostsFileObject' Contains 'Header'" -foreach $(Get-HostFile | Select-Object -First $headerCount) {
            $_.EntryType | Should -Be 'Header'
        }
        It "Test - First $headerCount Entries Should Not Match EntryType" -foreach $(Get-HostFile | Select-Object -First $headerCount | Get-HostFileEntryType -diag) {
            $_.Entry -Match $_.EntryType | Should -Be $false
        }
        It "Test - Entry after line $headerCount Should Match EntryType" -foreach $(Get-HostFile | Select-Object -Skip $headerCount | Get-HostFileEntryType -diag) {
